...
async function generateImage(imageUrl, payload) {
    try {
        ...
        console.log('content-type:', resp.headers['content-type'], 'size:', resp.data.byteLength); console.log('input payload:', payload);
        // rotateに渡す値を確認（今回は引数なし）
        console.log('sharp.rotate() will be called with no arguments');
        const rotatedBuffer = await sharp(Buffer.from(resp.data)).rotate().toBuffer(); let W, H;
        // 回転後のバッファからsharpインスタンスを作成
        const img = sharp(rotatedBuffer);
        // 回転後の画像のサイズを取得
        const meta = await img.metadata();
        if (!meta.format) throw new Error('Sharp が画像形式を認識できませんでした'); const { width: oW, height: oH } = meta;
        ...
        const payloadWithImage = {
            ...
            alwayson_scripts: {
                ...
                ...payload.alwayson_scripts,
                controlnet: {
                    // controlnet 設定があればそのまま argsWithImage を、なければ 空配列を設定                                                                                          ...payload.alwayson_scripts?.controlnet,
                    args: argsWithImage,
                },
            },
        };
        ...
        if (err.response) {
            console.error('APIエラー詳細:', JSON.stringify(err.response.data, null, 2)); console.error('APIエラーstatus:', err.response.status);
